...
//				}
	...
	_checkMsgSize: function (msg)
	{
		...
		var headersSize = msg.FromAddr.length + msg.ToAddr.length + msg.CCAddr.length
			+ msg.BCCAddr.value + msg.Subject.length * 2 + 400;
		
		var bodySize = (msg.HasHtm) ? newMsg.HtmlBody.length * 2 : msg.PlainBody.length * 2;
		
		var attachmentsSize = 0;
		for (var i = 0; i < msg.Attachments.length; i++) {
			var attachment = msg.Attachments[i];
			if (attachment) {
				attachmentsSize += parseInt(attachment.Size);
			}
		}
		
		var messageTotalSize = headersSize + bodySize + attachmentsSize;
		var availableMailboxSize = sett.MailBoxLimit - sett.MailBoxSize;
		if (availableMailboxSize < messageTotalSize)
		{
			return false;
		}
		return true;
	...
	Build: function (container, accountsBar, popupMenus)
	{
		...
		if (account.SaveMail != 0){
		    ...
		    lbl.innerHTML = "Also save in Sent Items";
