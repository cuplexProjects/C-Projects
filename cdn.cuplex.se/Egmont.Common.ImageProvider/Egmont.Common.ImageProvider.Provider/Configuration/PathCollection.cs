...
namespace Cuplex.Common.ImageProvider.Configuration
{
    ...
    public class PathCollection : ConfigurationElementCollection, IEnumerable<ImageProviderPath>
	{
		...
		///     collection.
		/// </returns>
		IEnumerator<ImageProviderPath> IEnumerable<ImageProviderPath>.GetEnumerator() => this.OfType<ImageProviderPath>().GetEnumerator();
		...
		/// </summary>
		/// <returns>A newly created <see cref="T:System.Configuration.ConfigurationElement" />.</returns>
		protected override ConfigurationElement CreateNewElement() => new ImageProviderPath();
		...
		protected override object GetElementKey(ConfigurationElement element) => ((ImageProviderPath)element).RoutePrefix;
		...
		protected override bool IsElementName(string elementName) => !string.IsNullOrEmpty(elementName) && elementName == ElementName;
	}
}