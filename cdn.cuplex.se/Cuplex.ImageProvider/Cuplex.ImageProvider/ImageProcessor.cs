	...
	internal static class ImageProcessor
	{
		...
		internal static bool ProcessImage(string targetPath, string originalPath)
		{
			...
			if (bildHantera.Contains("b"))
			{
				...
				if (tmp.IndexOfAny(delimiter) != -1)
				{
					tmp = tmp.Substring(0, tmp.IndexOfAny(delimiter));
				}
			...
			// Tag reda på önskad teknik för att storleksändra bilden.
			Teknik teknik;
			if (bildHantera.Contains("t"))
			{
				teknik = Teknik.Tvinga;
			}
			else if (bildHantera.Contains("u"))
			{
				teknik = Teknik.Utsnitt;
			}
			...
			{
				resizedImage = ResizeIndexedImage(img, maxBredd, maxHojd, teknik);
			}
			...
			{
				resizedImage = ResizeImage(img, maxBredd, maxHojd, teknik);
			}
			...
			if (rubrik.Length > 0)
			{
				...
				resizedImage = BildText_HS1(img, 30, rubrik, new Font("Arial", 16, GraphicsUnit.Pixel), new Point(5, 5));
		...
		/// <summary>
		///     Storleksändrar bild med en maxstorlek som påverkar både höjd och bredd beroende på val av Teknik
		/// </summary>
		/// <param name="image">Bild</param>
		/// <param name="width">Max Storlek Bredd</param>
		/// <param name="height">Max Storlek höjd</param>
		/// <param name="teknik">Välj vilken teknik för storleksändring som ska gälla.</param>
		/// <returns>Returnerar bildobjekt</returns>
		private static Image ResizeImage(Image image, float width, float height, Teknik teknik)
		{
			// Undersök om bilden redan är mindre än önskat
			if (teknik == Teknik.Max &&
			...
			// Storleksförändra beroende på vald teknik.
			if (teknik == Teknik.Max)
			{
			...
			if (teknik == Teknik.Tvinga)
			{
				...
				using (var graphics = Graphics.FromImage(bitmap))
				{
					...
					graphics.DrawImage(image, new Rectangle(-1, -1, (int)width + 2, (int)height + 2), 0, 0, image.Width, image.Height, GraphicsUnit.Pixel);
				}
			...
			else //( teknik == Teknik.Utsnitt )	// Eller Top
			{
				var bitmap = new Bitmap((int)width, (int)height);
				using (var graphics = Graphics.FromImage(bitmap))
				{
					...
					if ((float)image.Width / image.Height >= width / height)
					{
						...
						graphics.DrawImage(image, new Rectangle(-1, -1, (int)width + 2, (int)height + 2), widthDiff / 2, 0, newWidth, image.Height, GraphicsUnit.Pixel);
					}
					else if (teknik == Teknik.Top)
					{
						...
						graphics.DrawImage(image, new Rectangle(-1, -1, (int)width + 2, (int)height + 2), 0, 0, image.Width, newHeight, GraphicsUnit.Pixel);
					}
					else
					{
						...
						graphics.DrawImage(image, new Rectangle(-1, -1, (int)width + 2, (int)height + 2), 0, heightDiff / 2, image.Width, newHeight, GraphicsUnit.Pixel);
					}
				}
		...
		/// </returns>
		private static Image ResizeIndexedImage(Image originalImage, float maxWidth, float maxHeight, Teknik teknik)
		{
			Bitmap sourceBitmap;
			Bitmap targetBitmap;
			...
			// Storleksförändra beroande på vald teknik.
			if (teknik == Teknik.Max || teknik == Teknik.Tvinga)
			{
				if (teknik == Teknik.Max)
				{
				...
				targetBitmap = new Bitmap((int)maxWidth, (int)maxHeight, sourceBitmap.PixelFormat) { Palette = sourceBitmap.Palette };

				sourceData = sourceBitmap.LockBits(new Rectangle(0, 0, sourceBitmap.Width, sourceBitmap.Height), ImageLockMode.ReadOnly, sourceBitmap.PixelFormat);
				targetData = targetBitmap.LockBits(new Rectangle(0, 0, (int)maxWidth, (int)maxHeight), ImageLockMode.WriteOnly, targetBitmap.PixelFormat);
			...
			else //( teknik == Teknik.Utsnitt )  // Eller Top
			{
				...
				sourceData = sourceBitmap.LockBits(new Rectangle(0, 0, sourceBitmap.Width, sourceBitmap.Height), ImageLockMode.ReadOnly, sourceBitmap.PixelFormat);
				targetData = targetBitmap.LockBits(new Rectangle(0, 0, targetBitmap.Width, targetBitmap.Height), ImageLockMode.WriteOnly, targetBitmap.PixelFormat);
		...
		private static Image BildText_HS1(Image image, int extrabreddning, string text, Font font, Point point)
		{
			...
			using (var graphics = Graphics.FromImage(bitmap))
			{
				...
				graphics.DrawImage(image, new Rectangle(extrabreddning, 0, image.Width, image.Height), 0, 0, image.Width, image.Height, GraphicsUnit.Pixel);
