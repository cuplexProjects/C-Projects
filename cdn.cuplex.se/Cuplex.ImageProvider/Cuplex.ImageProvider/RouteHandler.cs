...
using Cuplex.ImageProvider.Configuration;
...
namespace Cuplex.ImageProvider
{
	...
	internal class RouteHandler : IRouteHandler, IHttpHandler
    {
        ...
        private void SetCacheAndSendFile(HttpContext context, string localPath, string originalPath,
            FileSystemInfo fileInfo)
        {
            ...
            if (localPath != null) context.Response.AddFileDependency(localPath);
        ...
        /// </summary>
        /// <param name="context">An <see cref="T:System.Web.HttpContext" /> object that provides references to the intrinsic
        /// server objects (for example, Request, Response, Session, and Server) used to service HTTP requests.</param>
        public void ProcessRequest(HttpContext context)
        {
            ...
            var pattern = new Regex(@"(?<g1>\/\/+)(\k<g1>)");
            if (pattern.IsMatch(path))
                foreach (var match in pattern.Matches(path))
                    path = path.Replace(match.ToString(), "/");


            DateTime? ifModifiedSince = null;
            var ifModifiedSinceHeader = context.Request.Headers["If-Modified-Since"];
            if (!string.IsNullOrEmpty(ifModifiedSinceHeader))
                if (DateTime.TryParse(ifModifiedSinceHeader, out var tmp))
                    ifModifiedSince = tmp;
            ...
            if (_applicationConfig.EnableFluentImage)
                originalFile =
                    path.Substring(0, path.LastIndexOf(path.Contains("__") ? "__" : ".", StringComparison.Ordinal)) +
