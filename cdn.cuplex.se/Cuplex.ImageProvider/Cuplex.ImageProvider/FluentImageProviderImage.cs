...
namespace Cuplex.ImageProvider
{
    ...
    public class FluentImageProviderImage
    {
        ...
        private ScalingType? _technique;
        private int? _width;
        ...
        /// <summary>
        /// Creates the specified image provider URL.
        /// </summary>
        /// <param name="imageProviderUrl">The image provider URL.</param>
        /// <returns></returns>
        /// <exception cref="ArgumentNullException">imageProviderUrl - Missing image URL.</exception>
        ...
        private string Render()
        {
            ...
            if (_width.HasValue) addUrl.Append($"b{_width.Value.ToString(CultureInfo.InvariantCulture)}");

            if (_height.HasValue) addUrl.Append($"h{_height.Value.ToString(CultureInfo.InvariantCulture)}");

            if (_technique == ScalingType.CutOut && _cropFromTop) _technique = ScalingType.Top;

            switch (_technique)
            {
                case ScalingType.Top:
                    addUrl.Append("f"); // Face - the top
                    break;
                case ScalingType.Max:
                    addUrl.Append("m");
                    break;
                case ScalingType.Force:
                    addUrl.Append("t");
                    break;
                case ScalingType.CutOut:
                    addUrl.Append("u");
                    break;
            }
            ...
            if (_rotateFlipType != RotateFlipType.RotateNoneFlipNone) addUrl.Append($"Rotera({_rotateFlipType})");

            if (!string.IsNullOrWhiteSpace(_special)) addUrl.Append($"Special({_special})");

            var imageUrl = _imageUrl;

            if (imageUrl.StartsWith("~")) imageUrl = _imageUrl.Substring(1);
