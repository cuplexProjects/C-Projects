...
namespace ASP_Web_API.Source
{
    internal class RandomGenerator
    {
        ...
        public string GenerateRandomPasswordString(int length)
        {
            int noBytes = length * 4;
            byte[] rndData = new byte[noBytes];
            using (var randomGenerator = RandomNumberGenerator.Create())
            ...
            StringBuilder sb = new StringBuilder();
            int poolLength = PASSWORD_CHAR_POOL.Length;
            for (int i = 0; i < noBytes; i += 4)
            {
                int randomArrayPos = Math.Abs(BitConverter.ToInt32(rndData, i)) % poolLength;
                sb.Append(PASSWORD_CHAR_POOL[randomArrayPos]);
            }
